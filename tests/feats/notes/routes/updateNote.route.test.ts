...
describe("Update Notes route test", () => {
  ...
  test("Deve retornar status 200 se receber as notas", async () => {
    jest.spyOn(UpdateNoteUseCase.prototype, "execute").mockResolvedValue(note);
    const result = await request(server).put("/note/user/1679411029731").send({
      noteTitle: "titulo",
      noteDescription: "descricao",
    });
    expect(result).toBeDefined();
    expect(result.body.message).toBe("Note updated");
